...
int main(int argc, char** argv) {
  ...
  if(argc == 7) {
        ...
        int_state_arr_num = atoi(argv[3])*sizeof(long long int);
        iv.int_state_arr = (long long int*)malloc(int_state_arr_num);
    ...
    while (sim_time--) {
        ...
        rob = -DBL_MAX;
        rob = MAX(rob, MAX(iv.x_arr[0] -  4.0, (-4.0) - iv.x_arr[0] ));
rob = MAX(rob, MAX(iv.x_arr[1] -  10.0, (1.5) - iv.x_arr[1] ));
rob = MAX(rob, MAX(iv.x_arr[2] -  20.0, (-20.0) - iv.x_arr[2] ));
